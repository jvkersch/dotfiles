...
* TODO elpy has a helpful feature that replaces if __name__ == '__main__' in Python scripts into a no-op (see https://github.com/jorgenschaefer/elpy/issues/69). I don't particularly care for this behavior, so set C-c C-c to calll the prefix form C-u C-c C-c by default.
...
** Don't forget to install CC mode from sourceforge (!)

* Package management.
  #+BEGIN_SRC emacs-lisp
  (require 'cask "/usr/local/Cellar/cask/0.8.1/cask.el")  ; Homebrew
  (cask-initialize)
  (require 'pallet)
  (pallet-mode t)
  #+END_SRC
...
(load "/Users/jvkersch/.opam/system/share/emacs/site-lisp/tuareg-site-file")
#+END_SRC
